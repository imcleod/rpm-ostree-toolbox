...
CMD ["/bin/sh", "/root/lorax.sh"]
    ...
    def createContainer(self, outputdir, post=None):
        ...
        lorax_cmd = ['lorax', '--nomacboot', '--add-template=/root/lorax.tmpl', '-p', os_pretty_name, '-v', os_v, '-r', os_v, " ".join(lorax_repos), '/out']
    ...
    def create(self, outputdir, post=None):
        ...
        imgfacbuild = ImgFacBuilder(verbosemode=self.verbosemode)
